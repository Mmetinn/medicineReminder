...
public class nabizSayActivity extends AppCompatActivity {
    ...
    ilacSaatiDB DB = new ilacSaatiDB(nabizSayActivity.this);
    private static final String NABIZSAY_ID="4";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_nabiz_say);
        TextView tx;
        ...
        etSeker = (EditText) findViewById(R.id.nabizEt);
        ...
        ArrayList<String> arrayList=DB.DBArrayOlcum(NABIZSAY_ID);
    ...
    private void createCardAll() {
        ...
        ArrayList<String>array=db.DBArrayOlcum(NABIZSAY_ID);
        int sayac=0;
        while (sayac<array.size()){
            String dizi[]=array.get(sayac).split("--");
            CardView cv = new CardView(nabizSayActivity.this);
    ...
    private DataPoint[] getDataPoint(){
        ilacSaatiDB DB = new ilacSaatiDB(this);
        DataPoint[]dp=DB.DBArrayOlcumXY(NABIZSAY_ID);
        return dp;
    }

    public void nabizKaydetClicked(View view){
        nabiz=etSeker.getText().toString();
        aciklama=etAciklama.getText().toString();
        tarih=dateFormatTx.format(new Date().getTime());

        olcum_class olcum = new olcum_class(nabiz,aciklama,NABIZSAY_ID,tarih);
