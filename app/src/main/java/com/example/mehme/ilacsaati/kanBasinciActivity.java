...
public class kanBasinciActivity extends AppCompatActivity {
    LineGraphSeries<DataPoint> series;
    SimpleDateFormat dateFormat= new SimpleDateFormat("dd MMM");
    SimpleDateFormat dateFormatTx= new SimpleDateFormat("dd MMM HH:MM");
    EditText etAciklama,etSeker;
    String kanBasinc,aciklama,tarih;
    ScrollView scrollView;
    GraphView graphView;
    LinearLayout linnear;

    ilacSaatiDB DB = new ilacSaatiDB(kanBasinciActivity.this);
    private static final String KANBASINCI_ID="5";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_kan_basinci);
        TextView tx;
        ...
        etSeker = (EditText) findViewById(R.id.kanBasinciEt);
        ...
        ArrayList<String> arrayList=DB.DBArrayOlcum(KANBASINCI_ID);
    ...
    private void createCardAll() {
        ...
        ArrayList<String>array=db.DBArrayOlcum(KANBASINCI_ID);
        int sayac=0;
        while (sayac<array.size()){
            String dizi[]=array.get(sayac).split("--");
            CardView cv = new CardView(kanBasinciActivity.this);
    ...
    private DataPoint[] getDataPoint(){
        ilacSaatiDB DB = new ilacSaatiDB(this);
        DataPoint[]dp=DB.DBArrayOlcumXY(KANBASINCI_ID);
        return dp;
    }

    public void kanBasincKaydetClicked(View view){
        kanBasinc=etSeker.getText().toString();
        aciklama=etAciklama.getText().toString();
        tarih=dateFormatTx.format(new Date().getTime());

        olcum_class olcum = new olcum_class(kanBasinc,aciklama,KANBASINCI_ID,tarih);
